...
| Profile status | **Supported**. | Effectively not supported[^1]. | Supported. |
...
| Attachments | Maximum 50 MB (maximum 500 MB for users with Nitro tier 2, and also for all users in a "server" with boost tier 3). | **Maximum 50~100 MB** (for most homeservers; customizable if you run your own homeserver). | Size limit depending on the server. |
...
| Notifications and notification settings | Mostly sender-controlled except for a small receiver-control of blocking the sender, server-side. | Reciever-controlled, server-side, including pattern matching. | Sender-controlled by default, additional receiver control planned, server-side. |
...
| VoIP in groups | Supported. | Supported. The room plays the role of signalling, audiovisual data is sadly routed through a separate WebRTC channel. | Supported. |
